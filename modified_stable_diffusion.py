...
class ModifiedStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    # Modify the gradient calculation here!!!
    @torch.no_grad()
    def __call__(
        self,
        ...
        for i, t in enumerate(timesteps):
            ...
            # grad = F.jacobian(self.unet, (latent_model_input, timestep, text_embeddings))
            # print(grad)
            # print("latent variable requires_grad is {}".format(latent_model_input.requires_grad))
            # grad = torch.autograd.grad(noise_pred, latent_model_input)
            # print(grad)

            # raise Exception("jacobian grad!")
        ...
        return ModifiedStableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept, init_latents=init_latents, latents=latents)
    ...
    # Enable gradient information.
    @torch.enable_grad()
    def generate_with_approx_grad(
        self,
        prompt: Union[str, List[str]],
        height: Optional[int] = None,
        width: Optional[int] = None,
        ...
        # expand the latents if we are doing classifier free guidance
        latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
        latent_model_input = self.scheduler.scale_model_input(latent_model_input, T).requires_grad_(True)
        ...
        # compute the previous noisy sample x_t -> x_t-1
        latents = self.scheduler.step(noise_pred, 0, latents, **extra_step_kwargs).prev_sample
        # print(latents.grad)
        # raise Exception("latent grad!")
    ...
    def encode_prompt(self, *args, **kwargs):
        """
        Compatibility wrapper for older diffusers versions that only expose
        `_encode_prompt`. Lets the rest of the code call `encode_prompt(...)`
        regardless of diffusers version.
        """
        return self._encode_prompt(*args, **kwargs)